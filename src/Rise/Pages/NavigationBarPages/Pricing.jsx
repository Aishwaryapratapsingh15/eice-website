...
import Pos from "../PricePages/PosSuite"
      ...
      <section className={`${styles.moduleSuiteSection} globalSectionSize`}  >
        ...
        <div className={`${styles.modulesTabHeadingContainer} font3`}>
          <div className={`${styles.modulesTabHeadings} font3`}>
            ...
            (
              <div style={{ padding: "7px 5px", cursor: "pointer", color: item.key === showSuitPage ? '#01B0F1' : 'rgb(156, 153, 153)', borderBottom: item.key === showSuitPage ? ' 2px solid #01B0F1' : '' }} onClick={(e) => { handleSuitPage(e, item.key), handleScroll() }} key={item.key ?? index}>{item.suiteName}</div>
            ))}
          </div>
        </div>
